 ...
 * @author rwitzel
 */
public class LightCouchCrudRepository<T, ID extends Serializable> implements CouchDbCrudRepository<T, ID> {

    protected Logger logger = LoggerFactory.getLogger(LightCouchCrudRepository.class);
    ...
    public LightCouchCrudRepository(Class<T> type, CouchDbClient couchDbClient) {
        this(type, false, couchDbClient, new GenericEntityInformation<T, ID>(type, (Class<ID>) String.class));
    }

    public LightCouchCrudRepository(Class<T> type, boolean allOrNothing, CouchDbClient couchDbClient,
